...
import { FaFeather, FaUserCircle } from 'react-icons/fa';
            ...
            <div id="main">
                ...
                <div className='middle-div'>
                    ...
                    <div className='mid-div'><div style={{ position: "relative" }}>
                        <div style={{ display: 'flex', gap: '500px' }}> <span>Home</span> 
                        <button type="button" className="btn btn-success" onClick={()=>logout()}>Logout</button></div>
                        <div id='home-btn' >
                            <ul className='navbar'>
                                <li ><Link to='/home'>My Tweets</Link></li>
                                <li id='active-tab'><Link to='/following'>All Tweets</Link></li>

                            </ul>
                        </div>
                        <button onClick={fetchPosts} id="refresh-btn" >Refresh</button>

                        <div id="my-tweets1">
                            {/* {JSON.stringify(tweets)} */}
                            {tweets.map((_tweet) => {
                                return (
                                    <div key={_tweet.id} style={{ borderBottom: '1px solid #dfcfcf', margin: '1rem' }}>
                                        <span style={{ fontSize: "30px" }}><FaUserCircle /></span> <span style={{ fontWeight: "bold" }}>{_tweet.author.username}</span>
                                        <p style={{ margin: "5px", color: 'grey' }}>{_tweet.tweet}</p>
                                    </div>
                                )
                            })}
                        </div>
                    </div>
                        <div className='input-div' style={{ display: "flex", gap: "20px", width: "634px", height: "60px" }}>
                            <ul>
                                <li>
